...
export function ReplayViewer() {
  ...
  const winProbability = useMemo<WinProbabilitySeries>(() => {
    ...
    return { status: "loading", model: "LightGBMFullDecay", provider: "lightgbm", cadenceSeconds: 0.5, experimental: true, points: [] };
            ...
            {selectedEngagement.participants.map((playerId) => {
              ...
              return (
                <article key={playerId} className={`engagement-player ${tradeLeader?.id === playerId ? "trade-leader-player" : ""}`} style={{ "--combat-color": player?.color ?? "#7b8794" } as React.CSSProperties}>
                  ...
                  <section className="engagement-actions"><header><span><ListTree size={11} />{t("watcher.observedActions")}</span><small>{t("watcher.factualEvents")}</small></header>{actions.length === 0 ? <small className="engagement-empty">{t("watcher.noActionsRecorded")}</small> : <ol>{actions.map((action, index) => <li key={`${action.time}-${action.unitType}-${index}`}><b>+{formatTime(action.time - selectedEngagement.start)}</b><span>{t("watcher.eliminated")} <strong>{entityName(action.unitType)}</strong></span><em>{compactNumber(action.mineralValue + action.vespeneValue)}</em></li>)}</ol>}</section>
                </article>
              );
            })}
          </div>
